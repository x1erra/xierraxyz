...
export default function VideoQueue({ queue, onAdd, onRemove, onPlayNext }: VideoQueueProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-900/90 backdrop-blur-xl border-r border-white/10 w-full h-full shadow-2xl font-sans font-light">
            {/* Header Section */}
            <div className="flex-none p-5 border-b border-white/5 bg-gradient-to-r from-black/40 to-transparent">
                ...
                <div className={`relative group transition-all duration-300 ${isFocused ? 'scale-[1.02]' : ''}`}>
                    <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
                        <LinkIcon size={14} className={`transition-colors ${isFocused ? 'text-purple-400' : 'text-zinc-600'}`} />
                    </div>
                    <input
                        className="w-full bg-black/40 border border-white/10 text-white text-xs pl-10 pr-10 py-3 rounded-xl focus:outline-none focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 transition-all placeholder-zinc-600 font-medium tracking-wide shadow-inner"
                    ...
                    <button
                        onClick={handleAdd}
                        className="absolute inset-y-0 right-2 my-1.5 px-2 bg-white/5 hover:bg-purple-500 text-zinc-500 hover:text-white rounded-lg transition-all border border-transparent hover:border-purple-400/50"
                    >
                        <Plus size={14} />
                    </button>
                </div>
                    ...
                    <AnimatePresence mode="popLayout">
                        {queue.map((item, i) => (
                            <motion.li
                                ...
                                key={`${item}-${i}`}
                                className="group relative flex items-center gap-3 bg-black/60 p-3.5 rounded-xl border border-white/5 hover:border-purple-500/40 transition-all hover:bg-zinc-800/80 hover:shadow-[0_4px_20px_-5px_rgba(168,85,247,0.15)]"
                            >
                                <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-gradient-to-br from-zinc-800 to-black border border-white/5 flex items-center justify-center text-[10px] text-zinc-500 font-mono group-hover:text-purple-400 group-hover:border-purple-500/30 transition-colors shadow-inner">
                                    {i + 1}
                                </div>
                                ...
                                <button
                                    onClick={() => onRemove(i)}
                                    className="opacity-0 group-hover:opacity-100 focus:opacity-100 p-2 hover:bg-red-500/10 text-zinc-500 hover:text-red-400 rounded-lg transition-all transform translate-x-2 group-hover:translate-x-0 focus:translate-x-0 border border-transparent hover:border-red-500/20 focus:outline-none focus:ring-2 focus:ring-red-500/20"
